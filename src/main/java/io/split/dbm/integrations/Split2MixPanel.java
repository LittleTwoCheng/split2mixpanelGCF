...
public class Split2MixPanel implements HttpFunction {
  ...
  public void service(HttpRequest request, HttpResponse response)
    ...
    Impression[] impressions = new Gson()
    .fromJson(new InputStreamReader(gzIs), Impression[].class);
    System.out.println(
      "successfully parsed " +
      impressions.length +
      " impressions in " +
    ...
    System.out.println(
      "encoded MixPanel events in " +
      (System.currentTimeMillis() - startMixPanel) +
      "ms"
    );
    ...
    System.out.println(
      "POSTed MixPanel. Response Body: " + apiResponse.body().string()
    );
    System.out.println(
      "POSTed MixPanel events in " +
      (System.currentTimeMillis() - startPost) +
      "ms"
    );

    PrintWriter writer = new PrintWriter(response.getWriter());
    writer.println(
      "" + impressions.length + " impressions accepted and posted to MixPanel"
    );
    writer.flush();
    writer.close();
    System.out.println(
